...
function InPlayGameState:enter()
    ...
    MOAIOuyaAndroid.setListener ( MOAIOuyaAndroid.OUYA_BUTTON_DOWN,  InPlayGameState.onOuyaButtonDownEvent )
...
function InPlayGameState:exit()
    ...
    MOAIOuyaAndroid.setListener ( MOAIOuyaAndroid.OUYA_BUTTON_DOWN, nil )
...
function InPlayGameState.onOuyaButtonDownEvent(keycode,player)
    if ( INPLAY_GAME_STATE == nil)
    then
        return
    end
-- OUYA BUTTONS
    ...
    then
        INPLAY_GAME_STATE:tryChangePacmanDirection( DIRECTION_UP )
    elseif ( keycode == BUTTON_DPAD_LEFT and player == 0)
    then
        INPLAY_GAME_STATE:tryChangePacmanDirection( DIRECTION_LEFT )
    elseif ( keycode == BUTTON_DPAD_DOWN and player == 0)
    then
        INPLAY_GAME_STATE:tryChangePacmanDirection( DIRECTION_DOWN )
    elseif ( keycode == BUTTON_DPAD_RIGHT and player == 0)
    then
        INPLAY_GAME_STATE:tryChangePacmanDirection( DIRECTION_RIGHT )
    elseif ( keycode == BUTTON_MENU and player == 0)
    then
        GAME_STATE_MACHINE:setCurrentState( PAUSE_GAME_STATE )
    end
end
