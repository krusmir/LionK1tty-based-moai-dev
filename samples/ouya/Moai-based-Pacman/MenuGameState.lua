...
--	MOAIInputMgr.device.touch:setCallback( MenuGameState.onTouchEvent )
    MOAIOuyaAndroid.setListener ( MOAIOuyaAndroid.OUYA_BUTTON_DOWN, MenuGameState.onOuyaButtonDownEvent )
...
--	MOAIInputMgr.device.touch:setCallback( nil )
    MOAIOuyaAndroid.setListener( nil )
end
...
function MenuGameState.onOuyaButtonDownEvent(keycode,player)
    if ( MENU_GAME_STATE == nil )
    then
        return
    end
